...
export const Route = createFileRoute('/recipe-tags/$id')({
  component: RecipeTagDetailPage,
})

function RecipeTagDetailPage() {
  const { id } = Route.useParams()
  const { data: row, isPending, error } = useQuery({
    queryKey: ['recipe_tags', id],
    queryFn: async () => {
      const { data, error } = await supabase.from('recipe_tags').select('*').eq('id', id).single()
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <article className="mx-auto max-w-3xl p-6 gap-6 py-12 space-y-8">
        ...
        <h1 className="text-4xl font-[family-name:var(--font-display)]">{String(row.id ?? 'Recipe Tag')}</h1>
        ...
        <Link to="/recipe-tags/"><Button variant="outline" className="transition-all duration-200">Back to Recipe Tags</Button></Link>
      </article>
    </div>
  )
}
